...
async function testCommunityRoutes() {
  ...
  try {
    // Test 1: Check if community routes are mounted
    console.log('Test 1: Check if community routes are mounted');
    try {
      const response = await axios.get(`${API_BASE_URL}/api/community`);
      console.log('✓ Community routes are mounted');
      console.log('Response:', response.data);
    } catch (error) {
      console.error('✗ Community routes not accessible:', error.response?.data || error.message);
